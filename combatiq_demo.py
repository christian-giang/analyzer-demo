...
if st.session_state['unlocked'] == True:
	...
	if subpage== 'pred':
		...
		picklefile = open('prediction_accuracy.pkl', 'rb')
		df_accuracy = pickle.load(picklefile)
		picklefile.close()


		acc_percentage = round(df_accuracy.iloc[0,1]/df_accuracy.iloc[0,0],2)

		col1, col2, col3 = st.columns(3)

		col1.metric(label="Prediction accuracy to date", value=str(int(acc_percentage*100))+"%")
		col2.metric(label="Fights predicted", value=str(df_accuracy.iloc[0,8]))
		col3.metric(label="Events covered", value=str(df_accuracy.iloc[0,9]))

		st.progress(acc_percentage)

		st.info("The prediction accuracy is only computed for fights with i) sufficient data available and ii) a minimum difference of 0.03 for the confidence values of both fighters. Fights used to compute the prediction accuracy are marked as (*). Predicted winners are marked as (P). For past fights, the actual winners are marked as (W), draws are marked as (D), fights that did not take place are marked as (N/A).")
		...
		enough_data = st.checkbox('Show only fights used for prediction accuracy computation')
		...
		if pred_choice == 'Upcoming fights':
			...
			if enough_data:
				df_pred = df_pred.loc[(df_pred['RC_nrecords']>df_accuracy.iloc[0,7]-1) & (df_pred['BC_nrecords']>df_accuracy.iloc[0,7]-1) & (abs(df_pred['RC_proba'] - df_pred['BC_proba']) > df_accuracy.iloc[0,6])] #Thresholds saved in dataframe
			...
			list_preds(df_pred, df_accuracy, pred_choice)

		elif pred_choice == 'Past fights':

			picklefile = open('all_predictions_bsv_results.pkl', 'rb')
			df_pred = pickle.load(picklefile).round(decimals=2)
			picklefile.close()	

			if enough_data:
				df_pred = df_pred.loc[(df_pred['RC_nrecords']>df_accuracy.iloc[0,7]-1) & (df_pred['BC_nrecords']>df_accuracy.iloc[0,7]-1) & (abs(df_pred['RC_proba'] - df_pred['BC_proba']) > df_accuracy.iloc[0,6])]

			list_events = df_pred['Event'].unique()
			...
			for event_i in list_events:
				...
				df_event = df_pred[df_pred['Event']==event_i]

				st.markdown("""---""") 
				st.markdown("<h4 style='text-align: center; color:white;'> Predictions for "+ df_event.iloc[0,3] + " ("+ df_event.iloc[0,4]  +") </h4>", unsafe_allow_html=True)

				list_preds(df_event, df_accuracy, pred_choice)
